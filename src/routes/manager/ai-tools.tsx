...
import { Bot, Sparkle, Mail, Copy, Send, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/manager/ai-tools")({
  head: () => ({
    meta: [{ title: "Manager AI Tools & Summary — FinPilot AI" }],
  }),
...
function ManagerAIToolsPage() {
  ...
  const handleDraftEmail = async () => {
    ...
    setTimeout(() => {
      setEmailText(
        `Team,\n\nPlease review the latest risk assessment and AI recommendations for application APP-24817.\nKey actions: ${recommendations.map((r) => r.title).join(", ")}\n\nBest,\nManager Dashboard`
      );
      setGeneratingEmail(false);
      toast.success("Manager overview email drafted");
    }, 1000);
  };

  return (
    <PortalShell role="manager" title="AI Tools & Summary" subtitle="Generate AI insights, draft manager communications, and view actionable recommendations.">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Automated manager‑focused recommendations.</p>
                </div>
              </div>
                ...
                {recommendations.map((r, i) => (
                  <div key={i} className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-xs space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="rounded-full bg-success/15 px-2 py-0.5 text-[10px] font-bold text-success">{r.confidence}% confidence</span>
                    </div>
        ...
        <div className="lg:col-span-6 space-y-4">
          <div className="glass-strong rounded-3xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-display text-base font-semibold">AI Manager Email Draft</h3>
                <p className="text-xs text-muted-foreground">Compose overview email for stakeholder communication.</p>
              </div>
            </div>
            ...
            {emailText && (
              <div className="space-y-3 pt-2">
                ...
                <div className="flex items-center justify-end gap-2">
                  <Button size="sm" variant="outline" onClick={() => { navigator.clipboard.writeText(emailText); toast.success("Email copied to clipboard"); }} className="rounded-xl">
                    <Copy className="size-3.5 mr-1" /> Copy Text
                  </Button>
                  <Button size="sm" onClick={() => { toast.success("Email dispatched to team!"); setEmailText(""); }} className="rounded-xl bg-brand text-white">
